...
<head>
    ...
    <style>
        ...
        .separator-line {
		    border: none;
		    width: 90%;
		    border-bottom: 1px solid #ddd; /* 선 색상 */
		    margin: 10px 15px 30px; /* 위아래 여백 */
		}
    </style>
</head>
<body>
    ...
    <div class="scroll-content">
        <c:forEach var="link" items="${linkMessageUser}">
            ...
            <a href="${messageLinkUrl}">
                <div class="follow-list user-item" data-username="${link.user_nickname}" data-userid="${link.user_id}" >
                    <img src="${link.user_image_url}" alt="user-img">
                    <div class="user-info">
                        <div class="user-nickname">
                            <span>${link.user_nickname}</span>
                        </div>
                        <div class="user-id">
        ...
        <c:forEach var="follow" items="${followList}">
            <c:forEach var="link" items="${linkMessageUser}">
                <c:if test="${link.user_id != follow.follow_user_id}">
		            <c:url var="messageLinkUrl" value="/messageLinkGo">
				        <c:param name="u" value="${follow.follow_user_id}" />
				        <c:param name="l" value="${link_id}" />
				    </c:url>
		            <a href="${messageLinkUrl}">
