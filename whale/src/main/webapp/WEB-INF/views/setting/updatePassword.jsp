...
function validatePassword() {
	...
	if (!passwordRegex.test(newPassword)) {
		$("#password_hint").text("8자리 이상 입력").css("color", "red");
	} else {
		$("#password_hint").text("조건 만족").css("color", "green");
	}

	// 두 비밀번호가 일치하는지 검사
	if (newPassword === checkPassword) {
		$("#password_match_hint").text("일치").css("color", "green");
	} else {
		$("#password_match_hint").text("불일치");
	}
}
